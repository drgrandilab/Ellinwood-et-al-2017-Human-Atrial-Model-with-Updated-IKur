function output = Ellinwood_et_al_ham_ikur_drug_model(t,y,p,runType)

% This file, which describes the human atrial EC coupling, was built upon
% the code developed by Grandi et al.
...
% 101-109: IKur Markov model (w/ drug interaction) - see lines 684-748
...
drug_index = p(3); drug = p(4);
% ISO administration (0 or 1)
ISO = p(7);
% Acetylcholine concentration (uM)
Ach = p(8);
% IKur-block parameters 
drug_kur_index = p(9); % (-) Drug-binding model
drug_kur_conc = p(10); % (uM)
kon_kur =  p(11); % (1/(uM*ms))
koff_kur = p(12); % (1/ms)
pore_block = p(17); % fraction (between 0 and 1)
...
% Model for IKur (including drug-binding) 
IKur_M_flag = 1; % 1 for new Markov model, 0 for original Hodgkin-Huxley model

%% Model Parameters
% Constants
Temp = p(5); % 300 or 310 % [K]
R = 8314;       % [J/kmol*K]  
Frdy = 96485;   % [C/mol]  
FoRT = Frdy/R/Temp;
...
Nao = p(6); %140; % Extracellular Na  [mM]
...
GNa = 10*(1-0.1*AF);%*(1+0.2*EAD);  % [mS/uF]
...
% equations for activation;
gkur_hh = (1-IKur_M_flag)*(1.0-0.5*AF)*(1+2*ISO)* 0.045*(1+0.2*RA); %nS/pF maleckar 0.045
...
%% I_kur  (NEW MARKOV MODEL)
y_kur = y(100+1:100+9);
ydot_kur = ydot(100+1:100+9);

% Conductance
gkur_m = (1 - pore_block)*IKur_M_flag*(1.0-0.5*AF)*(1+2*ISO)*0.045*1.36*(1+0.2*RA); %nS/pF maleckar 0.045*1.36 based on Feng physiological data (Figure 4)

% TRANSITION RATES (a1_ur, b1_ur, kfur, kbur)
% C4ur to O in ms^-1
a1_ur = 2.65*exp((y(39) - 75.9)./70); %modified model new
% C2ur to C1ur in ms^-1
b1_ur = 2.65*exp((y(39) - 75.9)./70) .* (exp(-(y(39) + 20.9)./10) ./ ...
    (1 + 0.3.*exp(-(y(39) + 20.9)./10)));
% O to I in ms^-1
kf_ur = 0.0005;
% I to O in ms^-1
kinact = 1./(1 + exp((y(39)+10)./10));
kb_ur = 0.0005*kinact;
...
elseif drug_kur_index == 1 % OPEN ONLY
    ydot_kur(1) = b1_ur*y_kur(2) - a4_ur*y_kur(1);
    ydot_kur(2) = a4_ur*y_kur(1) + b2_ur*y_kur(3) - y_kur(2)*(b1_ur + a3_ur);
    ydot_kur(3) = a3_ur*y_kur(2) + b3_ur*y_kur(4) - y_kur(3)*(b2_ur + a2_ur);
    ydot_kur(4) = a2_ur*y_kur(3) + b4_ur*y_kur(5) - y_kur(4)*(b3_ur + a1_ur);
...
elseif drug_kur_index == 2 % INACTIVE ONLY
    ydot_kur(1) = b1_ur*y_kur(2) - a4_ur*y_kur(1);
    ydot_kur(2) = a4_ur*y_kur(1) + b2_ur*y_kur(3) - y_kur(2)*(b1_ur + a3_ur);
    ydot_kur(3) = a3_ur*y_kur(2) + b3_ur*y_kur(4) - y_kur(3)*(b2_ur + a2_ur);
    ydot_kur(4) = a2_ur*y_kur(3) + b4_ur*y_kur(5) - y_kur(4)*(b3_ur + a1_ur);
...
elseif drug_kur_index == 3 % ClOSED ONLY 
...
elseif drug_kur_index == 4 %Open and Inactive (Foot-in-the-door Mechanism)
    ydot_kur(1) = b1_ur*y_kur(2) - a4_ur*y_kur(1);
    ydot_kur(2) = a4_ur*y_kur(1) + b2_ur*y_kur(3) - y_kur(2)*(b1_ur + a3_ur);
    ydot_kur(3) = a3_ur*y_kur(2) + b3_ur*y_kur(4) - y_kur(3)*(b2_ur + a2_ur);
    ydot_kur(4) = a2_ur*y_kur(3) + b4_ur*y_kur(5) - y_kur(4)*(b3_ur + a1_ur);
...
elseif drug_kur_index == 5 % (dO --> dI horizontal transition, for reviewer)
    ydot_kur(1) = b1_ur*y_kur(2) - a4_ur*y_kur(1);
    ydot_kur(2) = a4_ur*y_kur(1) + b2_ur*y_kur(3) - y_kur(2)*(b1_ur + a3_ur);
    ydot_kur(3) = a3_ur*y_kur(2) + b3_ur*y_kur(4) - y_kur(3)*(b2_ur + a2_ur);
    ydot_kur(4) = a2_ur*y_kur(3) + b4_ur*y_kur(5) - y_kur(4)*(b3_ur + a1_ur);
...
elseif drug_kur_index == 6 %Open and Inactive (Foot-in-the-door Mechanism), Variable Affinity
    ydot_kur(1) = b1_ur*y_kur(2) - a4_ur*y_kur(1);
    ydot_kur(2) = a4_ur*y_kur(1) + b2_ur*y_kur(3) - y_kur(2)*(b1_ur + a3_ur);
    ydot_kur(3) = a3_ur*y_kur(2) + b3_ur*y_kur(4) - y_kur(3)*(b2_ur + a2_ur);
    ydot_kur(4) = a2_ur*y_kur(3) + b4_ur*y_kur(5) - y_kur(4)*(b3_ur + a1_ur);
    ydot_kur(5) = a1_ur*y_kur(4) + kb_ur*I_ur + p(14)*y_kur(6) - y_kur(5)*(b4_ur + kf_ur + p(13)*drug_kur_conc);
    % I_ur
    ydot_kur(6) = y_kur(5)*p(13)*drug_kur_conc - y_kur(6)*p(14); %drug bound OPEN state
    ydot_kur(7) = p(15)*drug_kur_conc*I_ur - y_kur(7)*p(16); %drug bound to INACTIVE state
    
end
ydot(100+1:100+9) = ydot_kur;

I_kur_m = gkur_m*y_kur(5)*(y(39)-ek);
...
I_K_tot = I_to+I_kr+I_ks+I_ki-2*I_nak+I_CaK+I_kp+I_kur+I_KAch;     % [uA/uF] %SVP: added IKur
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%% Simulation type
% can create different protocols here using the format below
protocol = 'pace_cc'; % pace w/ current injection at rate 'rate'
...
% currents = [I_Na I_nabk I_nak I_kr I_ks I_kp I_tos I_tof I_ki I_ClCa I_Clbk I_Catot I_ncx I_pca I_cabk J_serca*Vmyo/Vsr];
    currents = [I_Na I_KAch I_Catot I_ncx vmax I_ki I_kur];
    output = currents;
end